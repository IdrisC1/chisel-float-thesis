...
module fpnew_divsqrt_multi #(
  ...
  input  fpnew_pkg_snax::operation_e       op_i, // Div or sqrt 
  input  fpnew_pkg_snax::fp_format_e       dst_fmt_i, // Destianation dataype format
  input  TagType                      tag_i,  
  input  logic                        mask_i, 
  // input  AuxType                      aux_i, // Not needed for now TODO : check if needed later
  input  logic                        vectorial_op_i, // Dertermine if verctorial operation -> SIMD

  // Input Handshake // Keep for now TODO: remove if not needed
  ...
  // Output handshake  // Keep for now TODO: remove if not needed 
  ...
  // Constants
  // ----------
  // Pipelines TODO Maybe remove pipelines later
  localparam NUM_INP_REGS = (PipeConfig == fpnew_pkg_snax::BEFORE)
                            ? NumPipeRegs
                            : (PipeConfig == fpnew_pkg_snax::DISTRIBUTED
                               ? (NumPipeRegs / 2) // Last to get distributed regs
                               : 0); // no regs here otherwise
  localparam NUM_OUT_REGS = (PipeConfig == fpnew_pkg_snax::AFTER || PipeConfig == fpnew_pkg_snax::INSIDE)
                            ? NumPipeRegs
                            : (PipeConfig == fpnew_pkg_snax::DISTRIBUTED
  ...
  fpnew_pkg_snax::roundmode_e rnd_mode_q;
  fpnew_pkg_snax::operation_e op_q;
  fpnew_pkg_snax::fp_format_e dst_fmt_q;
  logic                  in_valid_q;
  ...
  always_comb begin : translate_fmt
    unique case (dst_fmt_q)
      fpnew_pkg_snax::FP32:    divsqrt_fmt = 2'b00;
      fpnew_pkg_snax::FP64:    divsqrt_fmt = 2'b01;
      fpnew_pkg_snax::FP16:    divsqrt_fmt = 2'b10;
      fpnew_pkg_snax::FP16ALT: divsqrt_fmt = 2'b11;
      default:            divsqrt_fmt = 2'b10; // maps also FP8 to FP16
    endcase

    // Only if FP8 is enabled
    input_is_fp8 = FpFmtConfig[fpnew_pkg_snax::FP8] & (dst_fmt_q == fpnew_pkg_snax::FP8);
  ...
  assign div_valid   = ((in_valid_q & in_ready & ~flush_i) | ext_op_start_q) & (op_q == fpnew_pkg_snax::DIV);
  assign sqrt_valid  = ((in_valid_q & in_ready & ~flush_i) | ext_op_start_q) & (op_q != fpnew_pkg_snax::DIV);
