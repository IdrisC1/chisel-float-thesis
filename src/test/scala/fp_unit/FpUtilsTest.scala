...
class FpUtilsTest extends AnyFlatSpec with Matchers with FpUtils {
  ...
  it should "convert zeros correctly for all FP types" in {
    for (fpType <- fpTypes) {
      ...
      for (v <- zeros) {
        ...
        withClue(s"$typeName zero $v: uint=$uint back=$back") { back should (be(0.0f) or be(-0.0f)) }
      }
    }
  }
  ...
  it should "convert subnormals correctly for all FP types" in {
    for (fpType <- fpTypes) {
      ...
      for (v <- subnormals) {
        val uint = floatToUInt(fpType, v)
        val back = uintToFloat(fpType, uint)
        withClue(s"$typeName subnormal $v: uint=$uint back=$back") { back.isNaN shouldBe false }
      }
    }
  }
  ...
  it should "round-trip uint->float->uint for all FP types" in {
    for (fpType <- fpTypes) {
      ...
      for (_ <- 0 until numTests) {
        val uint = BigInt(fpType.width, r)
  ...
  it should "perform exact round-trip conversions for FP32" in {
